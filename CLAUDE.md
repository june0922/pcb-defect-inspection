...
\- \*\*Always Check Path Rules:\*\* Be hyper-vigilant about path consistency. Whenever you write or modify code, you must ensure that all paths use relative structures (not absolute paths). This guarantees that the project can be moved between any PC, server, or directory without breaking.
...
\- \*\*Runtime Injection Pattern:\*\* If a tool (e.g., ultralytics YOLO) requires an absolute path at runtime, inject it dynamically in code (e.g., `Path(...).resolve()`) rather than hardcoding it in config files. Use a placeholder like `PLACEHOLDER\_SET\_BY\_SCRIPT` in static config files to make this intent clear.
...
\- \*\*Single Source of Truth:\*\* NEVER hardcode configuration values (such as model names, training epochs, batch sizes, hyperparameters, or thresholds) directly in the source code.

\- \*\*Use Config Files:\*\* Always fetch these settings from centralized configuration files (e.g., `config.yaml`).

\- \*\*Propagation:\*\* The codebase must be designed so that changing a value in `config.yaml` automatically propagates to all related scripts (train, tune, evaluate, web demo, etc.) without requiring manual code modifications.
