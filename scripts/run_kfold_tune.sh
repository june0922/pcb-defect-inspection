#!/bin/bash
# YOLOv8 K-Fold Cross Validation Script (Hyperparameter tuned)
...
read -p "Proceed with K-Fold cross validation (tuned)? (Y/N, default is N): " run_kfold
...
python src/train_kfold_tune.py --config config.yaml $KFOLD_EXTRA
if [ $? -ne 0 ]; then
    echo ""
