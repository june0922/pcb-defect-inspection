@echo off
chcp 65001 >nul
title YOLOv8 Model Training (Hyperparameter tuned)
...
set TRAIN_CMD=python src\train_tune.py --config config.yaml
if not exist "runs\train_tune\weights\last.pt" goto start_train
...
if /I "%do_resume%"=="Y" (
    set TRAIN_CMD=python src\train_tune.py --config config.yaml --resume
)
echo.
