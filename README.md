...
⚠️ **필수 안내: Git LFS 설치**
이 프로젝트는 대용량 모델 가중치 파일(`.pt`)을 관리하기 위해 Git LFS를 사용합니다. 저장소를 다운로드(Clone)하거나 최신 버전을 받아오실 때 가중치 파일이 정상적으로 받아지려면 **[Git LFS](https://git-lfs.com/)**가 로컬 PC에 반드시 설치되어 있어야 합니다. (미설치 시 `.pt` 파일이 1KB 미만의 텍스트 포인터로 받아져 실행 시 에러가 발생합니다.)
...
4. [설정 파일 상세](#설정-파일-상세)
5. [소스 코드 상세](#소스-코드-상세)
6. [데스크톱 GUI 앱 (`app/`)](#데스크톱-gui-앱-app)
7. [웹 데모 — FastAPI 기반 검사 서버](#웹-데모--fastapi-기반-검사-서버)
8. [웹 뷰어 — 모델 성능 비교 도구](#웹-뷰어--모델-성능-비교-도구)
9. [실행 스크립트](#실행-스크립트)
10. [실행 가이드](#실행-가이드)
11. [스모크 테스트](#스모크-테스트)
...
│
├── app/                     # PySide6 기반 데스크톱 GUI 애플리케이션
│   ├── run.py               # 앱 실행 진입점
│   ├── main_ui.py           # 메인 윈도우 UI 및 이벤트 핸들링
│   ├── vision_viewer.py     # 이미지 뷰어 (줌/팬, BBox 오버레이)
│   ├── inference_worker.py  # 백그라운드 YOLO 추론 스레드
│   └── README.md
│
├── scripts/                 # 실행 보조 스크립트
│   ├── run_app.bat / .sh    # 데스크톱 GUI 앱 실행
...
### `visualize.py` — 시각화 관련 스크립트 삭제/이동됨
*(주석 생성 기능은 웹 데모나 GUI 앱에서 통합 처리)*

---

## 데스크톱 GUI 앱 (`app/`)

PySide6를 기반으로 개발된 데스크톱 이미지 뷰어 애플리케이션입니다. 

### 주요 기능
- **이미지 검사**: 이미지를 열고 `weights/best.pt` 모델로 추론하여 결함 영역(BBox) 시각화
- **상세 제어**: 마우스 휠 줌, 드래그 팬(Pan) 기능, 마우스 오버 시 툴팁을 통한 결함 정보(클래스, 신뢰도) 제공
- **리스트 연동**: 좌측 결함 목록 패널과 이미지 상의 BBox를 동기화하여 선택 시 상호 강조 표시
...
bash scripts/run_app.sh  # Linux/Mac
```
...
### `scripts/run_app.bat` / `run_app.sh`
- 데스크톱 GUI 애플리케이션을 실행합니다. PySide6 및 관련 의존성이 필요합니다.
...
# 7. 데스크톱 GUI 앱 실행
scripts\run_app.bat  # 또는 bash scripts/run_app.sh
