...
3. **학습/튜닝 스크립트 실행 (GPU 번호 지정 필수)**
   ```bash
   # 예: 0번 GPU를 사용하여 하이퍼파라미터 튜닝 실행
   CUDA_VISIBLE_DEVICES=0 bash scripts/run_tune.sh
   ```
   *(스크립트 실행 시 전처리 여부(Y/N) 및 튜닝 진행 여부를 묻는 프롬프트가 나타납니다.)*
...
4. **학습 결과물 내 로컬 PC로 다운로드 (SCP 활용)**
   ⚠️ 서버가 아닌 **내 로컬 PC(윈도우)의 새로운 PowerShell/CMD 창**에서 실행하세요.
   ```cmd
   # runs 폴더(학습 로그 및 세부 결과) 전체 다운로드
   scp -r team03@165.246.170.53:~/workspace/pcb-defect-inspection/runs ~/Downloads

   # weights 폴더(최종 산출된 가중치 파일들) 전체 다운로드
   scp -r team03@165.246.170.53:~/workspace/pcb-defect-inspection/weights ~/Downloads
   ```
