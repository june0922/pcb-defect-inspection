...
│   ├── run.py               # 앱 실행 진입점
│   ├── run_app.bat          # Windows: app_back + app_front 동시 실행
│   ├── run_app.sh           # macOS/Linux: app_back + app_front 동시 실행
...
│   └── README.md
│
├── app_back/                # [최종 산출물] REVIEW 타일 수동 판정 리뷰 스테이션
│   ├── run.py               # 앱 실행 진입점
│   ├── main_ui.py           # 메인 윈도우 (DB 폴링, 판정 단축키)
│   ├── vision_viewer.py     # 결함 오버레이 대화형 뷰어
│   ├── inference_worker.py  # 앙상블 모델 재추론 워커
│   └── README.md
│
...
프로젝트 내의 모든 모듈(`src`, `app_front`, `app_back`)은 `config.yaml`의 설정을 기반으로 동작합니다. 경로 지정, 데이터 분할 비율, 학습 에폭(Epoch) 등 시스템 전반의 핵심 파라미터를 여기서 일괄 관리합니다.
...
- **하이퍼파라미터 튜닝 (`tune.py`):** Ray Tune을 활용하여 최적 학습 파라미터를 탐색합니다.
...
3. **K-Fold 앙상블 학습**
   ...
   ```
4. **앱 실행 (app_front + app_back 동시 시작)**
   아래 스크립트 하나로 Review Station(app_back)과 Inspection Monitor(app_front)가 순서대로 시작됩니다.
   ```bash
   app_front\run_app.bat
   ```
   - app_back이 먼저 시작되어 5개 YOLO 모델을 로딩합니다 (약 30초~1분 소요).
   - 3초 후 app_front가 시작됩니다. `File > Open Folder...`로 검사 폴더를 선택하세요.
...
DB에서 3초마다 REVIEW 타일을 폴링하여 수신하고, 5개 모델로 재추론한 결과를 표시합니다.
