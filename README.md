...
본 프로젝트의 **최종 산출물**로, 학습된 AI 모델이 1차적으로 탐지해 낸 결함 후보들을 인간 작업자가 최종적으로 검토하고 판정하는 기능을 제공합니다. 

`weights/` 폴더 내의 **앙상블 가중치 5개**를 모두 로드하여 자체적인 교차 검증 및 NMS(Non-Maximum Suppression)를 수행, 예측의 신뢰도를 대폭 높였습니다.

### 직관적인 판정 UX
작업자의 반복적인 마우스 조작 피로도를 없애기 위해 키보드 기반의 빠른 판정 UX를 지원합니다.
* AI가 표시한 결함 후보를 확대 화면(Local View)으로 확인
* 키보드 **`F`** (Fail - 실제 불량) 또는 **`Space`** (Pass - 먼지/정상) 를 눌러 즉각 판정
* 방향키(`←`, `→`)로 다음/이전 결함 후보를 순회
* 최종 판정 결과는 JSON 파일로 리포팅되어 후속 통계나 재학습 데이터로 활용 가능

> 앱의 세부 화면 구성 및 단축키 안내는 `app/README.md`를 참고하시기 바랍니다.
