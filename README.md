...
│   ├── tune.py              # YOLOv8 하이퍼파라미터 자동 튜닝 (유전 알고리즘)
│   ├── evaluate.py          # test 세트 mAP / recall 평가
│   ├── pcb_inspect.py       # 보드 판정 레이어 (OK / NG / REVIEW)
│   └── visualize.py         # EDA + 검사 결과 시각화
├── scripts/
│   ├── run_train.bat / run_train.sh          # 통합 학습 실행 스크립트
│   ├── reset_to_main.bat / reset_to_main.sh  # main 브랜치 리셋
│   └── show_config.py                        # 현재 config.yaml 설정 출력
├── web/
│   ├── app.py               # FastAPI 서버 (단일 이미지 + 보드 검사 API)
│   ├── static/              # 프론트엔드 (HTML / CSS / JS)
│   ├── samples/             # 데모용 보드 이미지
│   └── tools/
│       └── build_demo_boards.py
├── weights/                 # 모델 가중치 (.pt)
└── notebooks/
...
# 2-2. 하이퍼파라미터 자동 튜닝 (선택 사항 — 시간이 매우 오래 걸림)
python src/tune.py
