...
│   ├── class_band_table.py  # 클래스별 REVIEW 밴드 테이블 위젯
│   ├── model_paths_widget.py# 검사 모델 선택 위젯
│   ├── defaults_store.py    # 공장 기본값 관리
│   ├── default_settings.json# 공장 기본값 설정
│   └── README.md
│
...
│   ├── train_kfold_tune.py  # 튜닝 결과 적용 5-Fold K-Fold 앙상블 학습
│   ├── utils.py             # config 로드 + 환경별 경로 분기
│   └── README.md
│
├── weights/                 # 베이스 모델 및 앙상블 최종 가중치
│   ├── yolo26s.pt           # YOLO26 Small 베이스 (~19.5 MB, 실제 사용)
│   ├── best_fold_1_tune~5_tune.pt # 5-Fold K-Fold 최적 가중치 (~19.6 MB × 5)
│   └── README.md
│
...
- **튜닝된 K-Fold 모델 학습 (`train_kfold_tune.py`):** `train_kfold.py`와 동일한 구조에 `config.yaml`의 `kfold_tune` 섹션(GA 튜닝 하이퍼파라미터)을 적용하는 별도 파이프라인입니다. 기존 `train_kfold.py`/`weights/best_fold_1~5.pt`에는 영향을 주지 않으며, 결과는 `runs/kfold_tune/`과 `weights/best_fold_1~5_tune.pt`로 분리 저장됩니다.
...
6. **학습/튜닝 스크립트 실행 (GPU 번호 지정 필수)**
   ...
   # [옵션 D] 튜닝된 하이퍼파라미터 기반 5-Fold 앙상블 학습 실행 (기존 옵션 C에 영향 없음)
   CUDA_VISIBLE_DEVICES=0 bash scripts/run_kfold_tune.sh
   ```
...
## 7. 최신 앙상블 성능 지표 (Test Set)
- **모델**: 26s_yestune_721_5kfold (500 Epochs, Hyperparameter Tuned)
- **앙상블 방식**: 5-Fold WBF (Weighted Boxes Fusion)
- **검출 성능**:
  - **mAP@0.5**: 0.9931
  - **mAP@0.5:0.95**: 0.8780
  - **Recall**: 0.9078

> 이 프로젝트의 `app_front/models/`에는 위 튜닝된 5개의 가중치가 기본 내장되어 있어, 설정 없이 최고 성능의 앙상블 추론을 제공합니다.
