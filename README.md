...
4. **기존 학습 결과 초기화 (선택)**
   새로운 학습을 시작하기 전에 이전에 생성된 학습 결과와 로그를 깨끗하게 지우려면 다음 명령어를 실행합니다. (필요한 결과는 미리 로컬로 다운로드하세요)
   ```bash
   rm -rf runs weights
   ```

5. **학습/튜닝 스크립트 실행 (GPU 번호 지정 필수)**
...
7. **학습 결과물 내 로컬 PC로 다운로드 (SCP 활용)**
   ⚠️ 서버가 아닌 **내 로컬 PC(윈도우)의 새로운 PowerShell/CMD 창**에서 실행하세요.
   ```cmd
   # runs 폴더(학습 로그 및 세부 결과) 전체 다운로드
   scp -r team03@165.246.170.53:~/workspace/pcb-defect-inspection/runs C:\Users\compu\Downloads

   # weights 폴더(최종 산출된 가중치 파일들) 전체 다운로드
   scp -r team03@165.246.170.53:~/workspace/pcb-defect-inspection/weights C:\Users\compu\Downloads
   ```
