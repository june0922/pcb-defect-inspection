...
| `pcb_tune_colab.ipynb` | 하이퍼파라미터 탐색 | O | X |
...
1. pcb_tune_colab.ipynb
   └─► Ray Tune으로 최적 하이퍼파라미터 탐색
         └─► 결과를 config.yaml에 반영하기

2. (tune 미활용 시) pcb_kfold_colab.ipynb  (또는 pcb_train_colab.ipynb)
   └─► 5-Fold 학습 → weights/best_fold_1~5.pt 생성
         └─► 이 가중치가 app_front에서 사용됨

3. (tune 활용 시) pcb_kfold_tune_colab.ipynb
...
2. 런타임 유형을 최소 **L4 GPU 이상**으로 설정 (A100 추천)
3. 첫 번째 셀부터 순서대로 실행 (Drive 마운트 → 패키지 설치 → 학습) (전체 실행 눌러도 됨)
* Google Drive 경로 본인 Drive 경로와 일치하는지 확인 필요
